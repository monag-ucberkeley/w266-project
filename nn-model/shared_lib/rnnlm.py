...
class RNNLM(object):
    ...
    def BuildCoreGraph(self):
        ...
        '''
        I studied this implementation and used what I learned there as well.
        https://github.com/lverwimp/tf-languagemodel
        '''
        ...
        with tf.name_scope("Embedding_Layer"):
            ...
            self.input_x_ = tf.reshape(tf.nn.embedding_lookup(params=(self.W_in_), 
                                                              ids=self.input_w_), 
        ...
        with tf.name_scope("Recurrent_Layer"):
            ...
            self.outputs_, self.final_h_ = tf.nn.dynamic_rnn(self.cell_, self.input_x_, 
        ...
        with tf.name_scope("Output_Layer"):                                   
            ...
            self.b_out_ = tf.Variable(tf.random_uniform([self.V], 0.0, 1.0), name="b_out_")
            # logits/scores = Tensor of size [batch_size X max_time X vocab_size]
            self.logits_ = tf.reshape(matmul3d(self.outputs_, self.W_out_) 
                                      + self.b_out_, [self.batch_size_, self.max_time_, self.V])
    ...
    def BuildTrainGraph(self):
        ...
        with tf.name_scope("train_model"):
            ...
            self.train_step_ = self.optimizer_.minimize(self.train_loss_, global_step=self.global_step)
    ...
    def BuildSamplerGraph(self):
        ...
        xx = tf.reshape(self.logits_, [-1, self.V])
        print xx.get_shape()
        self.pred_samples_ = tf.reshape(tf.multinomial(logits=tf.reshape(self.logits_, [-1, 1]), num_samples=1), [self.batch_size_, self.max_time_, 1], name="pred_random")
